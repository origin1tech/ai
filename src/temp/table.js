...
(function () {
	...
	var table = angular.module('ai.module.table', ['ngSanitize']);

	/* table provider merely provides a way to globally confgure ai-table */
	table.provider('$table', [ function $table() {
		...
		var self = this,
			...
			defaults = {
				...
				tableTemplate: 'ai-table-table.html',       // the template for the table datatype: string
		...
		this.$get = ['$templateCache', function $get($templateCache) {
			...
			tableTemplate =
				...
				'</thead>' +
				'<tbody>' +
				'<tr class="ai-table-row" ng-repeat="row in filtered" ng-click="selectTableRow(row, $event)" ng-if="$index >= filteredRows.start && $index < filteredRows.end" ng-class="{ \'ai-table-row-selected\': row.selected,  \'ai-table-row-editing\': row.edits }">' +
	...
	table.factory('TableFact', ['$rootScope','$http', '$q', '$templateCache', '$compile', '$table', '$filter', '$timeout', function TableFact($rootScope, $http, $q, $templateCache, $compile, $table, $filter, $timeout) {

		var isHtml, isPath, find, range, mapTo, filterObject, dragSupported;
		...
		mapTo = function mapTo (from, to, levels) {
			var ctr = 1;
			to = to || {};
		...
		dragSupported = function dragSupported() {
		...
		function Table(options) {
			...
			var loadTemplate, scope, bindScope, init, table,
				normalize, bootstrapTemplate, loadSource, changeDisplay,
				hasPrev, hasNext, pageTo, filter, sort, sortClass,
				userTemplates, loadedTemplates, onBind, selectAllRows,
				reset, applyFilter,	pagePrev, pageNext,	applyOrderBy,
				initOrderBy, mergeColumns, bind, initPager, loader,
				filterEvents, loadedPage, setFilteredRows, pageToKeyUp,
				selectRow, deleteRow, findRow, editRow, bindMethods, loading,
				selectTableRow, editRowCancel, self, isReady, ready, exportURI;
			...
			***********************************************/
			...
			loadTemplate = function loadTemplate(t) {
			...
			userTemplates = function userTemplates(promises) {
			...
			bootstrapTemplate = function bootstrapTemplate(template) {
			...
			**********************************************/
			...
			bind = function bind(callback, rebind) {
			...
			***************************************************/
			...
			mergeColumns = function mergeColumns(row) {
			...
			*****************************************************/
			...
			/* filters records */
			applyFilter = function applyFilter(q) {
			...
			*****************************************************/
			...
			/* orders records */
			applyOrderBy = function applyOrderBy(orderBy, rows) {
			...
			**************************************************/
			...
			pageToKeyUp = function pageToKeyUp(event, pg) {
				var keyCode = event.which || event.keyCode;
				if(keyCode === 13)
					pageTo(pg);
			};

			pagePrev = function pagePrev(pg) {
				if(!hasPrev(pg)) return;
				pageTo(pg -1);
			};

			pageNext = function pageNext(pg) {
				if(!hasNext(pg)) return;
				pageTo(pg +1);
			};

			/* checks if has a previous page */
			hasPrev = function hasPrev(pg) {
				pg = pg || scope.page;
				return pg - 1 > 0;
			};

			/* checks if has next page */
			hasNext = function hasNext(pg) {
			...
			**************************************************/
			...
			loadedPage = function loadedPage(pg) {
			...
			setFilteredRows = function setFilteredRows(pg) {
			...
			changeDisplay = function changeDisplay(disp) {
			...
			filterEvents = function filterEvents (obj, regex, asObject) {
				var objKeys = {},
					arrKeys = [],
			...
			**************************************************/
			...
			selectTableRow = function selectTableRow(row, e) {

				if (scope.editing || !scope.selectable) return;

				/* don't fire unless left click button */
				if(!e && e.button !== 0 && e.button !== 1) return;

				var target = angular.element(e.target);

				/* make sure our target is a column cell */
			...
			findRow = function findRow(key, value) {
			...
			editRow = function editRow(row, cancel) {
			...
			editRowCancel = function editRowCancel() {
				if(!scope.editing) return;
				scope.editing.edits = undefined;
				scope.editing = undefined;
			};

			/* fires when table has been bound */
			...
			**************************************************/
			...
			/*
			* Bind Scope
			* TODO: consider moving some properties locally to this context & use $emit or $broadcast
			*/
			...
			bindMethods = function bindMethods() {
				...
				self.getSource = function getSource() { return scope.source; };
				self.getSelected = function getSelected() { return scope.selected; };
				self.getEditing = function getEditing() { return scope.editing; };
				self.getOrderBy = function getOrderBy() { return scope.orderBy; };

				self.setRows = function setRows(arr) {
				...
				self.isDraggable = dragSupported();
				...
				self.sort = applyOrderBy;
				self.reset = reset;
				self.exportURI = exportURI;
			...
			*******************************************************/
			...
			initPager = function initPager(pg) {
			...
			init = function init() {
				...
				defaults = angular.copy($table.options);
				...
				templates.then(function (t) {
					...
					bind(function() {
						...
						tableTemplate = nodata ? loadedTemplates.nodata.template :
							'<div class="ai-table-wrapper table-responsive">' + loadedTemplates.loader.template + loadedTemplates.table.template + '</div>';
						...
						/* replace the table element with our new template */
						table = angular.element('<div class="ai-table"></div>');

						/* replace our oritinal element */
						options.element.replaceWith(table);
	 ...
	 */
	table.directive('aiTable', ['TableFact', function aiTable (TableFact) {
		...
		return {
			...
			link: function link(scope, element, attrs) {
				...
				scope.$watch('aiTable', function (newVal, oldVal) {
					...
					/* TODO allow updating of some options */
					//console.log(newVal);
				}, true);
	 ...
	 */
	table.directive('aiTableHeader', [ '$compile', function aiTableHeader ($compile) {
			...
			link: function link(scope, element, attrs) {
				...
				init = function init() {
					...
					if(!scope.column.header) {
						...
						value = $compile(scope.column.header)(scope);
						element.html(value);
					}
	 ...
	 */
	table.directive('aiTableRow', [function aiTable () {
...
//					}
	 ...
	 */
	table.directive('aiTableCell', [ '$compile', '$filter', '$parse', '$q', function aiTableCell ($compile, $filter, $parse, $q) {
		...
		return {
			...
			link: function link(scope, element, attrs) {
				 ...
				 * TODO: need to verify regex, maybe just apply and pass any args suppled by user.
				 */
				applyFilter = function applyFilter(val) {
				...
				compile = function compile() {
					...
					viewTemplate = $compile(viewTemplate)(scope);
